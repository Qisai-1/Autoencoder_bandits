...
# print('Cumulative explained variation for first_train_phase principal components: {}'.format(np.sum(pca_30.explained_variance_ratio_)))
...
class Decoder_soil_features(nn.Module):
    ...
    def __init__(self,
        ...
        #self.fc2 = nn.Linear(in_features=16, out_features=10)
        self.fc3 = nn.Linear(in_features=30, out_features=num_input_size)
...
class Autoencoder(nn.Module):
    ...
    def __init__(self,
        ...
        #self.encoder_onehot = encoder_class_one_hot(num_input_size_one_hot,latent_dim_one_hot)
        self.encoder_soil = encoder_soil_features(num_input_features,latent_dim_soil_features)
        #self.decoder_onehot = decoder_one_hot(num_input_size_one_hot,latent_dim_one_hot)
        self.decoder_soil = decoder_soil_features(decoder_output_size,latent_dim_soil_features + unique_seed)

    def forward(self, onehot, soil):
        soil_latent = self.encoder_soil(soil)
        combined_soil_onehot = torch.cat((soil_latent, onehot), dim=1)
        soil = self.decoder_soil(combined_soil_onehot)

        return  combined_soil_onehot, soil

def Dataset_split(obs):
    ...
    # one_hot_train_dataloader = DataLoader(one_hot_data_training, batch_size=batch_size, shuffle=True)
    # one_hot_val_dataloader = DataLoader(one_hot_data_val, batch_size=batch_size, shuffle=False)
    # one_hot_test_dataloader = DataLoader(one_hot_data_test, batch_size=batch_size, shuffle=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--latent_size', type=int, default=26, help='Size of the latent feature')
    ...
    feature_size = obs.shape[1] -1  - unique_seed# 1 is the yield column
    decoder_output_size = obs.shape[1] -1
    ...
    train_dataloader, val_dataloader, test_dataloader,  train_data, val_data , test_data = Dataset_split(obs)   
    criterion = nn.MSELoss()

    # Initialize the model, loss function, and optimizer
    latent_dim = latent_dim + unique_seed  
    ...
    for epoch in range(num_epochs):
        ...
        for batch_train in train_dataloader:
            ...
            if epoch < first_train_phase:
                ...
                loss_soil = criterion(soil_out, combined_soil_onehot)
                loss_soil.backward()
                optimizer.step()
        ...
        with torch.no_grad():
            ...
            for batch_val in val_dataloader:
                ...
                if epoch < first_train_phase:
                    ...
                    loss = criterion(soil_out, combined_soil_onehot)
                    val_loss += loss.item()
    ...
    latent_out, soil_out = autoencoder(val_total_data[:,:8], val_total_data[:,8:])
    ...
    # Create a scatter plot using a function and ax
    ...
    plt.savefig('plots/model_1_scatter.png')
    torch.save({
        'autoencoder': autoencoder,
        'linear_model': mapping
    }, 'models/model_1/Model_1_latent_addon_onehot.pth')
