...
def plot_loss_subplots(train_losses_list, val_losses_list,titles_list):
    ...
    # Show the plots
    plt.show()
    plt.savefig('plots/model_3_autoencoder_env_features.png')
...
if __name__ == "__main__":
    ...
    decoder_output_size = feature_size # feature size is the same as the output size reconstructed th environment features
    ...
    # soil_train_dataloader, soil_val_dataloader, soil_test_dataloader, one_hot_train_dataloader, one_hot_val_dataloader,one_hot_test_dataloader, train_data, val_data, test_data = Dataset_split(obs)
    # criterion = nn.MSELoss()
    train_dataloader, val_dataloader, test_dataloader,  train_data, val_data , test_data = Dataset_split(obs, batch_size)
    criterion = nn.MSELoss()

    # Initialize the model, loss function, and optimizer  
    hidden_size = latent_dim + unique_seed
    
    mapping = Linear_Mapping(hidden_size).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_train in train_dataloader:
            ...
            if epoch < first_train_phase:
                ...
                yield_result = mapping(combined_soil_onehot)
    ...
    latent_out, soil_out = autoencoder(val_total_data[:,8:])
    combined_soil_onehot = torch.cat((latent_out, val_total_data[:,:8]), dim=1)

    yield_pred = mapping(combined_soil_onehot)
    ...
    with torch.no_grad():
        ...
        for test_batch  in test_dataloader:
            ...
            combined_soil_onehot = torch.cat((latent_out, inputs_onehot), dim=1)
            yield_result_test = mapping(combined_soil_onehot)
    ...
    plt.savefig('plots/model_3_scatter.png')
    torch.save({
        'autoencoder': autoencoder,
        'linear_model': mapping
    }, 'models/model_3/Model_3_input_env_onehot.pth')
